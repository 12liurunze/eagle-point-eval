...
def ee_forward(input_ids, pixel_values, image_grid_thw, model, tokenizer, tree_choices, logits_processor=None, max_steps=512):
    ...
    for idx in range(max_steps):
        ...
        outputs = model.base_model(input_ids=input_id, use_cache=True, past_key_values=past_key_values)
...
def get_model_answers(
    ...
    for j in range(10):
        ...
        output_ids, new_token, idx = ee_forward(
            ...
            image_grid_thw=torch.as_tensor(image_grid_thw).cuda(),
            model=model,
            tokenizer=tokenizer,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ee-model-path",
        type=str,
        default="/home/dhz/tmp_model/EAGLE-EYE-Qwen2.5vl-7B-10k",
        help="The path to the weights. This can be a local folder or a Hugging Face repo ID.",
    )
    parser.add_argument("--base-model-path", type=str, default="/home/dhz/Qwen2.5-VL-7B-Instruct",
                        help="1")
    parser.add_argument(
        "--load-in-8bit", action="store_false", help="Use 8-bit quantization"
    )
    parser.add_argument("--model-id", type=str, default="Qwen2.5-VL-7B-Instruct-fp16-baseline")
    parser.add_argument(
        "--bench-name",
        type=str,
