...
parser.add_argument(
    "--outdir", type=str, default="/home/dhz/llava-instruct-output/get_data_llava_k_11"
)
...
bigname = "/home/dhz/llava-v1.5-7b-hf"

data = "/home/dhz/playground/data/coco/train2017"
...
def build_dataset_rank(
    ...
    ds = load_dataset(
        "json", data_files="/home/dhz/llava-instruct-150k/llava_instruct_150k.json"
    )
    ds = ds["train"]
...
def ge(data):
    ...
    hidden_state_k = outs_big.hidden_states[k]

    image_token_pos=torch.where(input_ids == bigmodel.config.image_token_index)
    first_pos=image_token_pos[1][0]
    image_token_len=len(image_token_pos[0])
    
    hidden_state_big[:,first_pos:first_pos+image_token_len]=hidden_state_k[:,first_pos:first_pos+image_token_len]
