...
class Model(nn.Module):
        ...
        if load_emb:
            ...
            if Type == "LlavaForConditionalGeneration":
                try:
                    ...
                    tensor = weights["language_model.model.embed_tokens.weight"].float()
                ...
                try:
                    ...
                    with safe_open(
                        os.path.join(path, emb_path), framework="pt", device="cpu") as f:
                        tensor_slice = f.get_slice(
                            "model.embed_tokens.weight"
                        )
                        vocab_size, hidden_dim = tensor_slice.get_shape()
            ...
            elif Type in ("PointLLMTreeForCausalLM", "PointLLMLlamaForCausalLM", "LlamaForCausalLM"):
                try:
                    with open(os.path.join(path, "model.safetensors.index.json"), "r") as f:
                        index_json = json.loads(f.read())
                ...
                try:
                    ...
                    raise ValueError(f"Unsupported model type for embedding loading: {Type}") from exc
    ...
    def forward(
        ...
        inputs_embeds = inputs_embeds.to(self.fc.weight[0].device)
        hidden_states = hidden_states.to(self.fc.weight[0].device)
        position_ids = position_ids.to(self.fc.weight[0].device) 
