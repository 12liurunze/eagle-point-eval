...
class EeModel(nn.Module):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if os.path.exists(configpath):
            print(f"[INFO] Loading config from local path: {configpath}")
        else:
            print(f"[INFO] Downloading config.json from Hugging Face repo: {ee_model_path}")
        ...
        if os.path.exists(load_model_path):
            print(f"[INFO] Loading model weights from local path: {load_model_path}")
        else:
            print(f"[INFO] Downloading model.safetensors from Hugging Face repo: {ee_model_path}")
    ...
    def forward(
        ...
        with torch.inference_mode():
            ...
            if pixel_values_videos is None and video_grid_thw is None :
