...
def initialize_tree(input_ids,model, tree_attn_mask, past_key_values, logits_processor, pixel_values=None, image_grid_thw=None,pixel_values_videos=None,video_grid_thw=None, point_clouds=None):
    ...
    model.language_model.tree_mask = tree_attn_mask
...
def update_inference_inputs(
    ...
    tree_logits = model.ee_layer.topK_genrate(accept_hidden_state_new,
                                              input_ids=torch.cat((input_ids, token.to(input_ids.device)), dim=1),
                                              model=model.base_model,
                                              head=model.lm_head, logits_processor=logits_processor)
